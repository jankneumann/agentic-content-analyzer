"""Add durable operation-observability correlation projections.
...
def upgrade() -> None:
    """Install additive, nullable correlation and bounded evidence storage."""

    op.execute(
        r"""
        ALTER TABLE pgqueuer_jobs
            ADD COLUMN root_job_id BIGINT NULL,
            ADD COLUMN submission_context JSONB NULL,
            ADD COLUMN submission_traceparent VARCHAR(512) NULL,
            ADD COLUMN submission_tracestate VARCHAR(512) NULL,
            ADD COLUMN trace_id CHAR(32) NULL,
            ADD COLUMN submission_span_id CHAR(16) NULL,
            ADD CONSTRAINT fk_pgqueuer_jobs_root_job_id
                FOREIGN KEY (root_job_id) REFERENCES pgqueuer_jobs(id) ON DELETE RESTRICT,
            ADD CONSTRAINT ck_pgqueuer_jobs_submission_context CHECK (
                submission_context IS NULL OR (
                    ...
                    AND length(submission_context->>'release_revision') BETWEEN 1 AND 64
                    AND octet_length(submission_context::text) <= 4096
                )
            ),
            ADD CONSTRAINT ck_pgqueuer_jobs_trace_id CHECK (
                trace_id IS NULL OR (
                    trace_id ~ '^[0-9a-f]{32}$' AND trace_id <> repeat('0',32)
                )
            ),
            ADD CONSTRAINT ck_pgqueuer_jobs_submission_span_id CHECK (
                submission_span_id IS NULL OR (
                    submission_span_id ~ '^[0-9a-f]{16}$'
                    AND submission_span_id <> repeat('0',16)
                )
            ),
            ADD CONSTRAINT ck_pgqueuer_jobs_submission_traceparent CHECK (
                submission_traceparent IS NULL OR (
            ...
            ADD CONSTRAINT ck_pgqueuer_jobs_context_identity CHECK (
                submission_context IS NULL OR (
                    ...
                    AND id IS NOT DISTINCT FROM
                        (submission_context->>'operation_id')::BIGINT
                    AND root_job_id IS NOT DISTINCT FROM
                        (submission_context->>'root_operation_id')::BIGINT
                    AND submission_traceparent IS NOT DISTINCT FROM
                        submission_context->>'traceparent'
                    AND submission_tracestate IS NOT DISTINCT FROM
                        submission_context->>'tracestate'
            ...
            ADD CONSTRAINT ck_pgqueuer_jobs_claim_generation_observable
                CHECK (claim_generation BETWEEN 0 AND 9223372036854775806);

        CREATE INDEX idx_pgqueuer_jobs_root_job_id ON pgqueuer_jobs(root_job_id);
        CREATE INDEX idx_pgqueuer_jobs_trace_id ON pgqueuer_jobs(trace_id)
        ...
        CREATE TABLE operation_observation_attempts (
            ...
            diagnostic_codes operation_diagnostic_code[] NOT NULL
                DEFAULT ARRAY[]::operation_diagnostic_code[],
            ...
            CONSTRAINT ck_operation_attempt_claim_generation
                CHECK (claim_generation BETWEEN 0 AND 9223372036854775806),
            CONSTRAINT ck_operation_attempt_number
                CHECK (attempt_number = claim_generation + 1),
            CONSTRAINT ck_operation_attempt_trace_id CHECK (
                trace_id ~ '^[0-9a-f]{32}$' AND trace_id <> repeat('0',32)
            ),
            CONSTRAINT ck_operation_attempt_root_span_id CHECK (
                root_span_id IS NULL OR (
            ...
            CONSTRAINT ck_operation_attempt_stage CHECK (
                terminal_stage IS NULL OR terminal_stage IN (
                    ...
                    'fallback','persist','index','graph','deliver','backup','restore',
                    'alert','cleanup','flush'
                )
            ),
            CONSTRAINT ck_operation_attempt_delivery CHECK (
                telemetry_delivery_state IN
                    ('pending','delivered','degraded','dropped','disabled')
            ),
            ...
            CONSTRAINT ck_operation_attempt_diagnostics_omitted
                CHECK (diagnostics_omitted >= 0)
        );
        CREATE INDEX idx_operation_attempts_trace_id
            ON operation_observation_attempts(trace_id);
        CREATE INDEX idx_operation_attempts_failure_completed
            ON operation_observation_attempts(completed_at)
            WHERE outcome IN ('partial','retryable_failure','permanent_failure');

        CREATE TABLE telemetry_process_health (
            environment VARCHAR(32) NOT NULL,
            service_name VARCHAR(100) NOT NULL,
            service_instance_id VARCHAR(128) NOT NULL,
            release_revision VARCHAR(64) NOT NULL,
            ...
            CONSTRAINT ck_telemetry_process_export_target
                CHECK (export_target IN
                    ('local_langfuse','remote_langfuse','other_otlp','none')),
            CONSTRAINT ck_telemetry_process_last_error_code CHECK (
                last_error_code IS NULL
                OR last_error_code ~ '^[a-z][a-z0-9_.-]{0,99}$'
            ),
            CONSTRAINT ck_telemetry_process_counts CHECK (
                buffered_count >= 0 AND buffer_capacity > 0
                AND buffered_count <= buffer_capacity AND dropped_count >= 0
            )
        );
        CREATE INDEX idx_telemetry_process_health_expiry
            ON telemetry_process_health(environment,expires_at,last_heartbeat_at DESC);
        CREATE INDEX idx_telemetry_process_health_freshness
            ON telemetry_process_health(
                environment,last_heartbeat_at DESC,service_name,service_instance_id
            );

        ALTER TABLE audit_log
            ADD COLUMN trace_id CHAR(32) NULL,
            ADD COLUMN request_span_id CHAR(16) NULL,
            ADD COLUMN submitted_operation_id BIGINT NULL,
            ADD COLUMN service_name VARCHAR(100) NULL,
            ADD COLUMN release_revision VARCHAR(64) NULL,
            ADD CONSTRAINT fk_audit_log_submitted_operation_id
                FOREIGN KEY (submitted_operation_id)
                REFERENCES pgqueuer_jobs(id) ON DELETE RESTRICT,
            ADD CONSTRAINT ck_audit_log_trace_id CHECK (
                trace_id IS NULL OR (
                    trace_id ~ '^[0-9a-f]{32}$' AND trace_id <> repeat('0',32)
                )
            ),
            ADD CONSTRAINT ck_audit_log_request_span_id CHECK (
                request_span_id IS NULL OR (
        ...
        CREATE INDEX idx_audit_log_trace_id ON audit_log(trace_id)
            WHERE trace_id IS NOT NULL;
        CREATE INDEX idx_audit_log_submitted_operation_id
            ON audit_log(submitted_operation_id)
            WHERE submitted_operation_id IS NOT NULL;
        CREATE INDEX idx_audit_log_trace_operation
        ...
        ALTER TABLE workflow_terminal_events
            ADD COLUMN trace_id CHAR(32) NULL,
            ADD CONSTRAINT ck_workflow_terminal_events_trace_id CHECK (
                trace_id IS NULL OR (
                    trace_id ~ '^[0-9a-f]{32}$' AND trace_id <> repeat('0',32)
                )
            );

        CREATE TABLE environment_ownership (
...
def downgrade() -> None:
    """Remove only the additive operation-observability objects."""

    op.execute(
        """
        DROP TABLE IF EXISTS environment_ownership;

        ALTER TABLE workflow_terminal_events
            DROP CONSTRAINT IF EXISTS ck_workflow_terminal_events_trace_id,
            DROP COLUMN IF EXISTS trace_id;

        DROP INDEX IF EXISTS idx_audit_log_trace_operation;
        DROP INDEX IF EXISTS idx_audit_log_submitted_operation_id;
        DROP INDEX IF EXISTS idx_audit_log_trace_id;
        ALTER TABLE audit_log
            DROP CONSTRAINT IF EXISTS ck_audit_log_request_span_id,
            DROP CONSTRAINT IF EXISTS ck_audit_log_trace_id,
            DROP CONSTRAINT IF EXISTS fk_audit_log_submitted_operation_id,
            DROP COLUMN IF EXISTS release_revision,
            DROP COLUMN IF EXISTS service_name,
            DROP COLUMN IF EXISTS submitted_operation_id,
            DROP COLUMN IF EXISTS request_span_id,
            DROP COLUMN IF EXISTS trace_id;

        DROP TABLE IF EXISTS telemetry_process_health;
        DROP TABLE IF EXISTS operation_observation_attempts;
        DROP DOMAIN IF EXISTS operation_diagnostic_code;

        DROP INDEX IF EXISTS idx_pgqueuer_jobs_trace_id;
        DROP INDEX IF EXISTS idx_pgqueuer_jobs_root_job_id;
        ALTER TABLE pgqueuer_jobs
            DROP CONSTRAINT IF EXISTS ck_pgqueuer_jobs_claim_generation_observable,
            DROP CONSTRAINT IF EXISTS ck_pgqueuer_jobs_context_identity,
            DROP CONSTRAINT IF EXISTS ck_pgqueuer_jobs_submission_traceparent,
            DROP CONSTRAINT IF EXISTS ck_pgqueuer_jobs_submission_span_id,
            DROP CONSTRAINT IF EXISTS ck_pgqueuer_jobs_trace_id,
            DROP CONSTRAINT IF EXISTS ck_pgqueuer_jobs_submission_context,
            DROP CONSTRAINT IF EXISTS fk_pgqueuer_jobs_root_job_id,
            DROP COLUMN IF EXISTS submission_span_id,
            DROP COLUMN IF EXISTS trace_id,
            DROP COLUMN IF EXISTS submission_tracestate,
            DROP COLUMN IF EXISTS submission_traceparent,
            DROP COLUMN IF EXISTS submission_context,
