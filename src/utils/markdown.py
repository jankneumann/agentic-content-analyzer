...
# Patterns for sections parsing
HEADING_PATTERN = re.compile(r"^(#{1,6})\s+(.+)$")
LIST_PATTERN = re.compile(r"^[-*+]\s+(.+)$")

# Patterns for extracting theme tags
CLEAN_ITEM_PATTERN = re.compile(r"\*\*|\*|`")
KEBAB_PATTERN = re.compile(r"[-_]")
CAMEL_PATTERN = re.compile(r"([a-z])([A-Z])")

# Patterns for normalizing keys
...
def extract_theme_tags(markdown: str) -> list[str]:
    ...
    for section in _flatten_sections(sections):
        if section.heading.lower() in ("key themes", "themes"):
            for item in section.items:
                ...
                clean = CLEAN_ITEM_PATTERN.sub("", item).strip()
    ...
    for match in HASHTAG_PATTERN.finditer(markdown):
        ...
        tag_clean = KEBAB_PATTERN.sub(" ", tag)
        tag_clean = CAMEL_PATTERN.sub(r"\1 \2", tag_clean)
