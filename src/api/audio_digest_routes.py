...
async def get_audio_digest_statistics() -> AudioDigestStatistics:
    ...
    """
    with get_db() as db:
        # OPTIMIZATION: Combine counts and sums into a single multi-dimensional query
        # This replaces 4 separate database round-trips with just 1.
        counts = (
            db.query(
                AudioDigest.status,
                AudioDigest.voice,
        ...
        total = 0
        status_map = dict.fromkeys(AudioDigestStatus, 0)
        by_voice = {}
        by_provider = {}
        total_duration = 0.0

        for status, voice, provider, count, duration_sum in counts:
            total += count
