...
def _run(
    ...
    *,
    ...
    # Persist the filter decisions. Committing is gated on dry_run only, not
    # on who owns the session: persisting its decisions is this hook's job,
    # and it always runs post-persist (the ingest adapter already committed
    # its rows), so there is no caller mid-transaction to surprise. A
    # caller-injected ``db`` (tests, future batch callers) is just an
    # injection seam. In dry-run mode nothing persists.
