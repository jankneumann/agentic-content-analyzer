...
- Does **not** keep architecture artifacts fresh for anyone else. They are a
  ...
  The reader ensures instead: the six consumer skills call
  `refresh-architecture/scripts/run_architecture.py --ensure` at the top of their
  artifact-reading step, and that call writes nothing when the check is fresh.
  This skill's `architecture` producer reports; it does not ensure on their
  behalf, and its result is informational drift in the gate.
- `scripts/checkpoint.py` is the same posture, deliberately. It records
