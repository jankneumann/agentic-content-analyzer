...
```bash
.agents/skills/use-railway/scripts/railway-api.sh '<query>' '<variables-json>'
```
...
`serializedConfig` is the raw JSON object from the template query, not a string. Get `workspaceId` via `.agents/skills/use-railway/scripts/railway-api.sh 'query { project(id: "<project-id>") { workspaceId } }' '{}'`.
