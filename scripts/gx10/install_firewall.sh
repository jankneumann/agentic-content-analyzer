...
for network in application stateful; do
  internal="$("${COMPOSE[@]}" config --format json | jq -r ".networks.${network}.internal")"
  [[ "$internal" == "true" ]] || {
...
for service in api worker scheduler maintenance; do
  if "${COMPOSE[@]}" config --format json | jq -e ".services.${service}.networks.egress" >/dev/null; then
...
if "${COMPOSE[@]}" config --format json | jq -e '[.services[] | .ports[]? | select(.host_ip != "127.0.0.1")] | length > 0' >/dev/null; then
