...
# A real down/up cycle exercises persisted host mounts and health ordering.
compose down --remove-orphans
prepare_runtime
compose up -d --wait
...
printf '%s\n' \
  '{"live":true,"registry_verified":true,"cold_restart_passed":true,"direct_routes_denied":true}' \
  >"$EVIDENCE"
chmod 0600 "$EVIDENCE"
