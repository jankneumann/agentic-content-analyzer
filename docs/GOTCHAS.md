...
| NUL bytes are rejected at the boundary, not per-driver | `NulByteGuardMiddleware` 422s any NUL in the path, query, **or JSON body**. Do not add per-driver handlers: psycopg2 raises a bare `ValueError` (unmapped → 500) while asyncpg raises `UntranslatableCharacterError` (a `DataError` → 422), so handler-side fixes need one per quirk. Body inspection is JSON-only — multipart file bytes may legitimately contain NUL. See `src/api/middleware/nul_byte_guard.py` |
| A 5xx is a claim about *your* health | Two halves of one rule. **Don't invent a cause you haven't established**: never map a blanket `except Exception` to 502/503/504: use them only for exceptions that prove it (scope the `try` to the provider call, or raise a dedicated type like `KBQAProviderError`), and let everything else reach the global handlers — `DataError` → 422, unknown → 500. **Don't call a settled condition transient**: see the `503`-for-a-disabled-feature row below. Both mislabels point triage at the wrong subsystem and tell clients to retry something that is fine |
...
| Schemathesis NUL bytes are now a real 422 | Schemathesis draws NUL for free text in query params *and* JSON bodies; `NulByteGuardMiddleware` rejects both with 422, so contract tests no longer need to skip them. The draw is random and the corpus has no `derandomize`, so consecutive full runs explore disjoint input — one green run is not proof a NUL defect is gone |
...
| Ingestion service results differ from transport results | Ingestion services may use `IngestionResponse` internally, but external CLI, HTTP, and MCP workflow mutations return a durable `OperationHandle`. Observe completion and results through the operations surface; do not treat command acceptance as completed ingestion |
| `URLIngestResult` and legacy MCP URL shapes are removed | Use `IngestionResponse.details["content_id"]` + `details["duplicate"]` inside ingestion services. External consumers should call the typed tools in `src/mcp_tools/ingestion.py` and follow the returned `OperationHandle`; there is no supported flat `{content_id, status, duplicate, source}` compatibility envelope |
