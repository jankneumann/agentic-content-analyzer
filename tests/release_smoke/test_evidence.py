...
def test_runtime_schema_matches_reviewed_openspec_contract() -> None:
    # The durable contract lives under openspec/contracts/ (parallel to
    # openspec/specs/) and remains authoritative after the originating change is
    # archived. Live code and tests must not depend on files below
    ...
    reviewed = json.loads(
        (
            REPO_ROOT / "openspec/contracts/release-smoke/release-smoke-evidence.schema.json"
        ).read_text()
    )
