...
from src.config.models import ModelConfig, Provider, ProviderConfig, ModelStep, ModelInfo, ModelFamily
...
@pytest.mark.asyncio
async def test_extract_themes_openai(theme_analyzer, mock_model_config, mock_newsletters_and_summaries):
    ...
    with patch("src.processors.theme_analyzer.OpenAI") as MockOpenAI:
        ...
        # Run extraction
        themes = await theme_analyzer._extract_themes_with_llm(
            newsletters, summaries, [], 10, 0.5
        )
...
@pytest.mark.asyncio
async def test_extract_themes_anthropic(theme_analyzer, mock_model_config, mock_newsletters_and_summaries):
    newsletters, summaries = mock_newsletters_and_summaries
...
@pytest.mark.asyncio
async def test_extract_themes_fallback(theme_analyzer, mock_model_config, mock_newsletters_and_summaries):
