...
def report() -> dict[str, Any]:
    """The full 17-scenario passing run. Mutate freely — each test gets a fresh copy."""
...
def test_the_offline_run_covers_less_and_is_still_complete() -> None:
    ...
    offline run evaluates 11 of 17 scenarios and is entirely credible, because 11 is
...
def test_a_below_threshold_run_fails_but_stays_credible(
    ...
    report.update(passed=13, failed=4, pass_rate=13 / 17)
...
def test_the_threshold_is_honoured_exactly(report: dict[str, Any]) -> None:
    report.update(passed=16, failed=1, pass_rate=16 / 17)
    assert validate(report, None, fail_threshold=16 / 17).ok
...
def test_the_script_separates_its_two_failures(tmp_path: Path) -> None:
    report = _load("report-full-pass.json")
    report.update(passed=0, failed=17, pass_rate=0.0)
