...
class TestRecommendations:
    ...
    def test_more_than_five_test_failures_trigger_fix_tests(self):
        findings = [_finding(id=f"tf{i}", category="test-failure") for i in range(6)]
        sr = _source(findings=findings)
        report = aggregate([sr])
    ...
    def test_exactly_five_test_failures_no_recommendation(self):
        findings = [_finding(id=f"tf{i}", category="test-failure") for i in range(5)]
        sr = _source(findings=findings)
        report = aggregate([sr])
    ...
    def test_more_than_ten_lint_findings_trigger_fix_scrub(self):
        findings = [_finding(id=f"l{i}", category="lint") for i in range(11)]
        sr = _source(findings=findings)
        report = aggregate([sr])
    ...
    def test_exactly_ten_lint_findings_no_recommendation(self):
        findings = [_finding(id=f"l{i}", category="lint") for i in range(10)]
        sr = _source(findings=findings)
        report = aggregate([sr])
    ...
    def test_recommendations_capped_at_five(self):
        ...
        test_failures = [_finding(id=f"tf{i}", category="test-failure") for i in range(6)]
        lint_findings = [_finding(id=f"l{i}", category="lint") for i in range(11)]
        deferred = [
            _finding(
                id=f"d{i}",
                category="deferred-issue",
