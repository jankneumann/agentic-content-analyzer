 ...
 *
 * Dialog for configuring and triggering newsletter summarization.
 * Allows selecting:
 * - Specific newsletter IDs or all pending
...
export function GenerateSummaryDialog({
  ...
  const [newsletterIds, setNewsletterIds] = React.useState("")
  const [force, setForce] = React.useState(false)

  const parsedIds = React.useMemo(() => {
    if (!newsletterIds.trim()) return []
  ...
  const canGenerate =
    mode === "pending" ? pendingCount > 0 : parsedIds.length > 0
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[450px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Summarize newsletters using AI to extract key insights.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-2">
            <Label>Newsletters to Summarize</Label>
          ...
          {mode === "pending" ? (
            <div className="rounded-lg border p-4 bg-muted/30">
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-medium">{pendingCount} newsletters pending</p>
                  <p className="text-xs text-muted-foreground">
                    Newsletters without summaries will be processed
                  </p>
                </div>
                {pendingCount > 0 && (
            ...
            <div className="space-y-2">
              <Label className="text-sm text-muted-foreground">
                Newsletter IDs (comma or space separated)
              </Label>
              <Input
                placeholder="e.g., 123, 456, 789"
                value={newsletterIds}
                onChange={(e) => setNewsletterIds(e.target.value)}
              />
              {parsedIds.length > 0 && (
          ...
          <Button onClick={handleGenerate} disabled={isGenerating || !canGenerate}>
            {isGenerating ? (
              ...
              <>
                ...
                Summarize {mode === "pending" ? `${pendingCount} Newsletters` : `${parsedIds.length} Newsletters`}
              </>
            )}
          </Button>
        </DialogFooter>
