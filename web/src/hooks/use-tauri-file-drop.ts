    ...
    async (paths: string[]) => {
      ...
      for (const filePath of paths) {
        ...
        if (validation.status !== "valid") {
          ...
          toast.error(
            validation.error || `Cannot upload ${validation.fileName}`
          )
          continue
        }
        ...
        if (uploadResult.ok) {
          if (uploadResult.operation) addOperation(uploadResult.operation)
          uploaded++
          ...
          toast.error(
            `Failed to upload ${validation.fileName}: ${uploadResult.error}`
          )
        }
      }
