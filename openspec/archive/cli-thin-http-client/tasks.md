...
- [x] **T1.9** Write tests: `tests/cli/test_api_client.py` (mock httpx transport), `tests/cli/test_progress.py`
- [x] **T1.10** Write tests: `tests/config/test_settings_api_url.py` (verify field defaults and profile loading)
...
- [x] **T2.8** Write tests: `tests/api/test_ingest_extended.py` (new source types), `tests/queue/test_worker_extended.py`
- [x] **T2.9** Write tests: `tests/ingestion/test_orchestrator_gmail_config.py` (verify sources.d reading)
...
- [x] **T3.7** Write tests: `tests/cli/test_ingest_http.py` (mock ApiClient, verify HTTP path for each source)
- [x] **T3.8** Update existing CLI tests to use `--direct` flag where they test direct behavior

## Phase 4: Pipeline API

- [x] **T4.1** Extract pipeline stage functions from `src/cli/pipeline_commands.py` to `src/pipeline/runner.py`
- [x] **T4.2** Create `src/api/pipeline_routes.py` with `POST /run` and `GET /status/{job_id}` endpoints
- [x] **T4.3** Register `run_pipeline` handler in `src/queue/worker.py` that calls `src/pipeline/runner.py`
- [x] **T4.4** Register pipeline router in `src/api/app.py`
- [x] **T4.5** Add `run_pipeline` entrypoint to `ENTRYPOINT_LABELS` in `src/models/jobs.py`
- [x] **T4.6** Refactor `src/cli/pipeline_commands.py` daily/weekly to: try API → fallback → direct
- [x] **T4.7** Write tests: `tests/api/test_pipeline_routes.py`, `tests/pipeline/test_runner.py`

## Phase 5: Summarize + Digest CLI

- [x] **T5.1** Add `summarize()` and `stream_summarize_status()` methods to `ApiClient`
- [x] **T5.2** Refactor `src/cli/summarize_commands.py` `summarize_pending` to call `POST /api/v1/contents/summarize`
- [x] **T5.3** Refactor `src/cli/summarize_commands.py` `summarize_id` to call same endpoint with `content_ids=[id]`
- [x] **T5.4** Add `create_digest()` method to `ApiClient`
- [x] **T5.5** Refactor `src/cli/digest_commands.py` to call `POST /api/v1/digests/generate`
- [x] **T5.6** Write tests for summarize and digest HTTP paths

## Phase 6: Remaining CLI Commands

- [x] **T6.1** Convert `src/cli/review_commands.py` (list, view, revise) to API calls
- [x] **T6.2** Convert `src/cli/analyze_commands.py` (themes) to API call
- [x] **T6.3** Convert `src/cli/podcast_commands.py` (generate, list-scripts) to API calls
- [x] **T6.4** Convert `src/cli/job_commands.py` (list, show, retry, history) to API calls
- [x] **T6.5** Convert `src/cli/settings_commands.py` (list, get, set) to API calls
- [x] **T6.6** Convert `src/cli/prompt_commands.py` (list, show, set, reset, test) to API calls
- [x] **T6.7** Write tests for all converted commands
