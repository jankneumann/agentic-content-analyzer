...
Section 2 (S2 client)    ─── no deps, parallel with 1, 3, 5
Section 3 (source config) ── no deps, parallel with 1, 2, 5
Section 4 (ingestion svc) ── depends on 1, 2, 3
Section 5 (ref extractor) ── depends on 2 (client)
Section 6 (orchestrator)  ── depends on 4, 5
Section 7 (CLI)           ── depends on 6
Section 8 (web search)    ── depends on 2 (client only)
Section 9 (API routes)    ── depends on 1, 4
Section 10 (frontend)     ── no deps, parallel with all
Section 11 (docs)         ── depends on all
```

**Max parallel width: 4** (sections 1, 2, 3, 5 can run concurrently in first wave; 10 anytime)
...
- [ ] 1.3 In the same migration, add GIN index on `metadata_json` using `op.execute()` raw SQL (Alembic's `op.create_index()` does not support `CONCURRENTLY`): `CREATE INDEX ix_content_metadata_json_gin ON content USING GIN (metadata_json jsonb_path_ops)`
- [ ] 1.4 Run migration and verify enum and index are available in database

## 2. Semantic Scholar Client (parallel with 1, 3, 5)
...
## 3. Source Configuration (parallel with 1, 2, 5)
...
## 4. Scholar Content Ingestion Service (depends on 1, 2, 3)
...
## 5. Reference Extraction (depends on 2; parallel with 1, 3)
...
## 6. Orchestrator & Pipeline Integration (depends on 4, 5)
...
- [ ] 7.3 Add `aca ingest scholar-refs` command with `--after`, `--before`, `--source`, `--dry-run`, `--limit` flags
- [ ] 7.4 Write CLI integration tests

## 8. Web Search Provider (depends on 2)
...
## 9. Source Routes & API (depends on 1, 4)
...
## 10. Frontend Source Type Registration (no deps, parallel with all)
...
## 11. Documentation & Configuration (depends on all)
