...
-- Obsidian local-vault state is deliberately digest-only. Raw paths, vault
-- roots, note bodies, frontmatter, URLs, and exception messages never cross
...
CREATE TABLE obsidian_ingest_state (
    ...
    CONSTRAINT ck_obsidian_ingest_state_shape CHECK (
        ...
        OR (status = 'claimed' AND claim_token IS NOT NULL
         AND lease_expires_at IS NOT NULL AND attempt_count >= 1 AND error_code IS NULL)
        OR (status = 'ingested' AND claim_token IS NULL
         AND lease_expires_at IS NULL AND error_code IS NULL)
        OR (status = 'failed' AND claim_token IS NULL
         AND lease_expires_at IS NULL AND error_code IS NOT NULL)
        OR (status = 'deferred' AND claim_token IS NULL
         AND lease_expires_at IS NULL AND error_code IS NOT NULL)
    )
);
...
CREATE TABLE obsidian_ingest_events (
    ...
    CONSTRAINT ck_obsidian_ingest_events_shape CHECK (
        (status = 'discovered' AND claim_token IS NULL
         AND lease_expires_at IS NULL AND operation_id IS NULL
         AND content_id IS NULL AND error_code IS NULL
         AND completed_at IS NULL)
        OR (status = 'claimed' AND claim_token IS NOT NULL
         AND lease_expires_at IS NOT NULL AND content_id IS NULL AND error_code IS NULL
...
-- GX-10 operation observability correlation projection.
