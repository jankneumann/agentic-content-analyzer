...
- Add a Substack subscription sync that updates `sources.d/substack.yaml` (Substack.yml) with enabled toggles for each subscription.
- Ingest the latest posts defined in `substack.yaml` (Substack.yml) and deduplicate posts by canonical Substack URL across RSS, Gmail, and Substack sources.
