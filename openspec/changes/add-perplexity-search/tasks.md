...
- [x] 1.1 Create `src/services/web_search.py` with provider protocol and types:
  ```python
  @dataclass
  ...
  ```
- [x] 1.2 Add `web_search_provider: str = "tavily"` to `src/config/settings.py` (env: `WEB_SEARCH_PROVIDER`, values: `tavily|perplexity|grok`)
...
- [x] 2.1 Create `TavilyWebSearchProvider` class implementing `WebSearchProvider`:
...
- [x] 2.2 Register `TavilyWebSearchProvider` in `get_web_search_provider()` factory for `provider="tavily"`

## 3. Configuration and Settings (Perplexity)

> Independent — no dependencies. Parallel with tasks 1 and 2.

- [x] 3.1 Add Perplexity configuration settings to `src/config/settings.py`:
...
- [x] 3.2 Add settings to `profiles/base.yaml` with `${PERPLEXITY_API_KEY:-}` interpolation
- [x] 3.3 Document environment variables in CLAUDE.md

## 4. Database Schema

> Independent — no dependencies. Parallel with tasks 1-3.

- [x] 4.1 Add `PERPLEXITY = "perplexity"` to `ContentSource` enum in `src/models/content.py`
- [x] 4.2 Create Alembic migration to add `perplexity` value to `content_source` enum type:
  ```sql
  ALTER TYPE contentsource ADD VALUE IF NOT EXISTS 'perplexity';
  ```
- [x] 4.3 Run migration locally and verify enum update
...
- [x] 5.1 Create `src/ingestion/perplexity_search.py` module with data models:
  ```python
  @dataclass
  ...
  ```
- [x] 5.2 Implement `PerplexityClient` class:
  ```python
...
- [x] 5.3 Implement `PerplexityResponse` Pydantic model:
  ```python
  class PerplexityResponse(BaseModel):
      ...
      usage: dict[str, int] = {}
  ```
- [x] 5.4 Implement response-to-ContentData conversion:
...
- [x] 5.5 Add error handling for API failures, auth errors, rate limits
- [x] 5.6 Add retry logic with exponential backoff for transient errors
...
- [x] 6.1 Implement `PerplexityContentIngestionService`:
  ```python
  class PerplexityContentIngestionService:
      ...
      def close(self) -> None
  ```
- [x] 6.2 Implement prompt retrieval via `PromptService`:
  - Key: `pipeline.perplexity_search.search_prompt`
  - Fallback to settings/default if not overridden
- [x] 6.3 Implement citation-based deduplication:
...
- [x] 6.4 Use SAVEPOINT isolation per search result (same pattern as xSearch)
- [x] 6.5 Track cost metadata: token usage, request count, context size

## 7. Default Search Prompt

> Independent — no dependencies. Parallel with other tasks.

- [x] 7.1 Add default search prompt to `src/config/prompts.yaml`:
  ```yaml
...
- [x] 8.1 Create `PerplexityWebSearchProvider` implementing `WebSearchProvider`:
...
- [x] 8.2 Register in `get_web_search_provider()` factory for `provider="perplexity"`
...
- [x] 9.1 Create `GrokWebSearchProvider` implementing `WebSearchProvider`:
...
- [x] 9.2 Register in `get_web_search_provider()` factory for `provider="grok"`
...
- [x] 10.1 Add `ingest_perplexity_search()` to `src/ingestion/orchestrator.py`:
  ```python
...
- [x] 11.1 Add `aca ingest perplexity-search` command to `src/cli/ingest_commands.py`:
...
- [x] 11.2 Implement human-readable output:
  - Show items ingested, skipped, queries made, citations found
  - Show errors if any
- [x] 11.3 Implement JSON output mode via `is_json_mode()` guard pattern
- [x] 11.4 Implement `on_result` callback to capture `PerplexitySearchResult`
...
- [x] 12.1 Update `src/processors/podcast_script_generator.py`:
...
- [x] 12.2 Update `src/api/chat_routes.py`:
...
- [x] 13.1 Create `sources.d/websearch.yaml` with example entries:
  ```yaml
  defaults:
    type: websearch
    enabled: true
  ...
  sources:
    ...
    - name: "AI Twitter Pulse"
      ...
      max_threads: 30
  ```
- [x] 13.2 Add `WebSearchSource` Pydantic model to `src/config/sources.py` (alongside existing `RSSSource`, `PodcastSource`, etc.):
  ```python
...
- [x] 13.3 Update pipeline `_run_ingestion_stage_async()` to read websearch sources:
...
- [x] 14.1 Create `tests/test_services/test_web_search.py`:
...
- [x] 14.2 Test `TavilyWebSearchProvider`:
...
- [x] 14.3 Test `PerplexityWebSearchProvider`:
...
- [x] 14.4 Test `GrokWebSearchProvider`:
...
- [x] 15.1 Create `tests/test_ingestion/test_perplexity_search.py`:
...
- [x] 16.2 Add CLI tests to `tests/cli/test_ingest_commands.py`:
  ...
  - Test help text
- [x] 16.3 Test pipeline integration:
...
- [x] 17.1 Test `WebSearchSource` model validation:
...
- [x] 18.1 Test podcast script generator uses `get_web_search_provider()`:
  - Mock provider factory
  - Verify `_handle_web_search()` delegates to configured provider
- [x] 18.2 Test chat routes `web_search_enabled` reflects any configured provider:
...
- [x] 19.2 Update `docs/SETUP.md` with Perplexity API key setup instructions
