...
- **New unified config file** (`sources.yaml`) consolidating all ingestion source definitions
...
- **Settings integration** — `sources.yaml` path configurable via `SOURCES_CONFIG_FILE` env var
- **Migration script** — convert existing config files + `AI-ML-Data-News.md` into `sources.yaml`
...
- Affected code:
  - `src/config/settings.py` — new `get_sources_config()` method, `SOURCES_CONFIG_FILE` setting
  - `src/config/sources.py` — new module: YAML parser, source models, migration utilities
  - `src/ingestion/rss.py` — consume sources from config, distinguish article vs podcast feeds
  - `src/ingestion/youtube.py` — support channel IDs and RSS feeds alongside playlists
  - `src/ingestion/podcast.py` — **new module**: podcast RSS ingestion with STT transcription
  ...
  - `sources.yaml` — new config file (project root)
