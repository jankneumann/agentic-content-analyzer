...
- Provide a cutover-ready GX-10 topology for the application services, PostgreSQL, graph database, Langfuse, ClickHouse, Redis, and MinIO with authenticated secrets, persistent volumes, health checks, restart policy, backup/restore, quotas, and retention controls sized for a 1 TB disk; production traffic and data ownership remain in the separate cutover change.
...
- **Operator surfaces:** exact operation and diagnostic APIs consumed by the existing CLI, health/readiness, local Compose/system service definitions, retention/storage dashboards, and runbooks. A new frontend observability UI is out of scope.
