...
Every nullable envelope key SHALL be present with an explicit null value when absent. Before a claim, attempt number SHALL be null; after a claim it SHALL equal claim generation plus one. Operation IDs, epochs, claim generations, attempt numbers, and cursors SHALL remain within signed 64-bit database bounds and SHALL be encoded as canonical decimal strings at JSON and JavaScript boundaries. Claimed generation SHALL NOT exceed 9223372036854775806 so `attempt_number = claim_generation + 1` cannot overflow. JSON Schema and OpenAPI SHALL enforce structure and scalar bounds but SHALL NOT be treated as sufficient ingress validation. Every consumer SHALL call a generated composite semantic validator before accepting or binding an envelope; raw TypeScript assertions and structural-only validation are forbidden. The envelope SHALL use canonical W3C version-00 Trace Context encodings, SHALL reject malformed or oversized untrusted values and all-zero trace or parent identifiers at ingress, SHALL require traceparent's trace and parent identifiers to equal the explicit envelope trace ID and span ID, SHALL validate a bounded W3C tracestate subset, and SHALL never contain credentials, raw source URLs, unrestricted prompts, unrestricted source content, or exception text.
