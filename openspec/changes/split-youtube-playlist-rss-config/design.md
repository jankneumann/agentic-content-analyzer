...
### Decision 6: Exponential backoff on 429 rate limiting

Both `youtube-transcript-api` (HTTP 429 from YouTube) and the YouTube Data API (`HttpError 429`) can return rate-limit errors. The system SHALL retry with exponential backoff before giving up on a video.
...
**Implementation:** Use `tenacity` library (already available as a transitive dependency via `google-api-python-client`) or a simple inline retry loop. Prefer inline loop to avoid adding a dependency purely for retry logic.
...
### Decision 7: Cloud OAuth via environment variable
