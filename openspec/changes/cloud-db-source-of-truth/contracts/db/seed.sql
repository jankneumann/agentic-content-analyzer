...
-- Covers one row per canonical audit shape: authed read, audited destructive write,
-- audited resolve, 5xx error with notes, 401 no-credentials, 403 invalid-key.
-- `admin_key_fp` policy: always the SHA-256 last-8 of the presented X-Admin-Key
-- (regardless of validity); NULL only when the header is absent.
...
INSERT INTO audit_log (timestamp, request_id, method, path, operation, admin_key_fp, status_code, body_size, client_ip, notes) VALUES
    ...
    ('2026-04-21T10:04:00Z', 'req-test-005', 'GET',     '/api/v1/audit',                  NULL,                       NULL,       401,   0,    '10.0.0.99', '{"auth_failure": "missing_key"}'::jsonb),
    ('2026-04-21T10:05:00Z', 'req-test-006', 'POST',    '/api/v1/kb/lint/fix',            'kb.lint.fix',              'zzzzzzzz', 403,   0,    '10.0.0.99', '{"auth_failure": "invalid_key"}'::jsonb)
;
