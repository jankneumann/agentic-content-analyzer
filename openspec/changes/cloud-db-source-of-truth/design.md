...
**`admin_key_fp` source**: Since audit runs outside auth, the fingerprint is computed from `request.headers.get("X-Admin-Key")` directly (SHA-256, last 8 chars), not from a validated key. This is intentional — invalid-key attempts deserve a fingerprint for correlation across requests.
...
| `extract_references` | `{scanned, references_found, dry_run}` | `{references_extracted, content_processed, per_content: [{content_id, references_found}]}` |
...
- **Request ID** — generated by existing `TraceMiddleware` (or `RequestIdMiddleware` if present); populated into `request.state.request_id` and echoed in `X-Request-Id` response header. `AuditMiddleware` reads from `request.state.request_id` so audit rows correlate with trace logs.
