...
## Purpose
Backend notification event system that emits events on pipeline job completions and failures, with event history, real-time delivery via SSE, device registration, and per-event-type preferences.
