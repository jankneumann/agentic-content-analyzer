...
- [ ] 1.1 Create `src/storage/providers/neon.py` with `NeonProvider` class
...
- [ ] 1.2 Update `src/storage/providers/factory.py`
...
- [ ] 1.3 Update `src/storage/providers/__init__.py`
  - Export `NeonProvider` class

- [ ] 1.4 Add Neon settings to `src/config/settings.py`
...
- [ ] 2.1 Create `src/storage/providers/neon_branch.py` with `NeonBranchManager` class
...
- [ ] 4.2 Create `tests/test_storage/test_neon_branch.py`
...
- [ ] 5.1 Create `tests/integration/fixtures/neon.py`
  - `neon_test_branch` fixture (module-scoped)
  - Skip if `NEON_API_KEY` not set
  - Cleanup branches after test session

- [ ] 5.2 Create example integration test using Neon branch
  - Demonstrate branch isolation for database tests
  - Show parallel test execution with separate branches

## 6. Documentation

- [ ] 6.1 Update `docs/SETUP.md` with Neon configuration section
  - Account creation and project setup
  - Connection string formats (pooled vs direct)
  - Environment variable configuration
  - Migration setup

- [ ] 6.2 Add agent workflow documentation
...
- [ ] 6.3 Update `CLAUDE.md` with Neon-specific guidance
  - When to use Neon vs local PostgreSQL
  - Branch management best practices

## 7. Validation

- [ ] 7.1 Run existing provider tests to ensure no regressions
