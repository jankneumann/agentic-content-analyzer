...
- [ ] 1.1 Add `crawl4ai>=0.7.0` to `pyproject.toml` as optional dependency group
  ```toml
  [project.optional-dependencies]
  crawl4ai = ["crawl4ai>=0.7.0"]
  ```
- [ ] 1.2 Add Settings fields to `src/config/settings.py`:
...
- [ ] 1.3 Add `${CRAWL4AI_*}` interpolation references in `profiles/base.yaml`
  ```yaml
  settings:
    crawl4ai:
  ...
  ```
- [ ] 1.4 Write unit tests for new Settings fields (`tests/config/test_settings_crawl4ai.py`)
  - Test defaults
...
- [ ] 2.1 Update `HtmlMarkdownConverter.__init__()` to read from `get_settings()`
...
- [ ] 2.2 Add CacheMode mapping in `src/parsers/html_markdown.py`
  ```python
  CACHE_MODE_MAP = {
...
- [ ] 2.3 Add `crawl4ai_excluded_tags` to `CrawlerRunConfig` in `_convert_with_crawl4ai()`
- [ ] 2.4 Add configurable `page_timeout` to `CrawlerRunConfig`
- [ ] 2.5 Add remote server mode: `_convert_with_crawl4ai_remote()`
...
- [ ] 2.6 Update `_convert_with_crawl4ai()` to dispatch local vs remote
  - If `self.server_url` is set → call `_convert_with_crawl4ai_remote()`
  - Else → use existing `AsyncWebCrawler` in-process path
- [ ] 2.7 Write unit tests for converter updates (`tests/parsers/test_html_markdown_crawl4ai.py`)
...
- [ ] 3.3 Register `crawl4ai` pytest marker in `pyproject.toml`
  ```toml
  ...
  ```
- [ ] 3.4 Add Crawl4AI server status to `/ready` health check response
...
- [ ] 4.1 Create Hoverfly simulation for Crawl4AI REST API
...
- [ ] 4.2 Create integration test fixture (`tests/integration/fixtures/crawl4ai.py`)
  - `crawl4ai_available` session-scoped fixture
  - `requires_crawl4ai` skip marker
  - `crawl4ai_url` fixture from settings
- [ ] 4.3 Write integration tests (`tests/integration/test_crawl4ai_extraction.py`)
...
- [ ] 4.4 Write live integration tests (require Docker)
...
- [ ] 5.1 Update `docs/SETUP.md` — add Crawl4AI section
...
- [ ] 5.2 Update `docs/ARCHITECTURE.md` — extraction pipeline diagram
  - Show two-tier flow: Trafilatura → quality check → Crawl4AI (local/remote)
- [ ] 5.3 Update `CLAUDE.md` — add gotchas
...
- [ ] 5.4 Update `docs/TESTING.md` — add Crawl4AI test section
