...
- [x] 1.1 Add backup settings to `Settings` class in `src/config/settings.py`:
...
- [x] 2.1 Create `railway/postgres/init-backup-job.sql`:
...
- [x] 2.2 Update `railway/postgres/init-extensions.sql` to source backup job setup
- [ ] 2.3 Verify backup job creates valid, restorable dumps

## 3. Documentation

- [x] 3.1 Add backup section to `docs/SETUP.md`:
...
- [x] 3.2 Add backup configuration to Railway deployment section in `docs/MOBILE_DEPLOYMENT.md`

## 4. Health Check Integration (Optional)

- [x] 4.1 Add backup recency check to `/ready` endpoint
...
- [x] 5.1 Add unit tests for backup settings validation
- [ ] 5.2 Add integration test for backup job creation (requires pg_cron)
- [x] 5.3 Verify backup job SQL syntax is valid
