...
- [x] 1.1 Create `railway/postgres/` directory structure
- [x] 1.2 Create `railway/postgres/Dockerfile` with multi-stage build:
...
- [x] 2.1 Create `.github/workflows/build-railway-postgres.yml`:
...
- [x] 3.1 Update `DatabaseProviderType` in `src/config/settings.py` to include `"railway"`
- [x] 3.2 Add Railway-specific settings to `Settings` class:
...
- [x] 3.3 Add Railway validation in `validate_database_provider_config()` model validator
- [x] 3.4 Add `get_effective_database_url()` case for Railway provider
- [x] 3.5 Create `src/storage/providers/railway.py` with `RailwayProvider` class:
...
- [x] 3.6 Update `src/storage/providers/factory.py` to handle `"railway"` provider type
- [x] 3.7 Update `src/storage/providers/__init__.py` to export `RailwayProvider`

## 4. Storage Provider Implementation

- [x] 4.1 Add Railway MinIO settings to `Settings` class:
  - `railway_minio_endpoint: str | None = None`
  - `railway_minio_bucket: str | None = None`
  - `minio_root_user: str | None = None`
  - `minio_root_password: str | None = None`
- [x] 4.2 Create `RailwayFileStorage` class in `src/services/file_storage.py`:
...
- [x] 4.3 Update `get_storage()` factory function to handle `"railway"` provider
...
- [x] 6.1 Add `TestRailwayProvider` class in `tests/test_storage/test_providers.py`:
...
- [x] 6.2 Add Railway storage tests in `tests/test_services/test_file_storage.py`:
...
- [x] 7.1 Update `CLAUDE.md` Database Providers table to include Railway with extensions
- [x] 7.2 Update `CLAUDE.md` File Storage Providers table to include Railway
- [x] 7.3 Add Railway setup section in `docs/SETUP.md`:
...
- [x] 7.5 Create `railway/README.md` with deployment guide (Note: placed in `railway/postgres/README.md`)
