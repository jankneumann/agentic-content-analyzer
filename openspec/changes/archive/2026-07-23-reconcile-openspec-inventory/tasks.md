...
- [x] 2.1 Verify the implemented filtering foundation and identify unsupported
  runtime, CLI, projection, feedback, and observability claims.
- [x] 2.2 Verify the implemented source-override foundation and identify
  request-contract, UI, docs, design, and migration-evidence gaps.
- [x] 2.3 Verify HuggingFace extraction plus canonical registry, contract, MCP,
  worker, capability, and frontend integration; reconcile its stale tasks.
- [x] 2.4 Verify the implemented LLM evaluation/routing foundation and enumerate
  the missing deployable evaluation-to-routing loop.
- [x] 2.5 Verify Langfuse/profile behavior and the public canonical ParadeDB
  image; separate missing documentation and production proof.
- [x] 2.6 Verify canonical MCP ingestion returns durable operation handles and
  that `unify-mcp-ingest-envelope` is superseded.
- [x] 2.7 Re-run or revalidate the recorded focused gates for completed RI-01
  and RI-02.
- [x] 2.8 Publish the per-change synchronization matrix naming exact retained
  requirements, omitted claims, merge strategy, archive mode, and tests.

## 3. Actionable successors and retained changes

- [x] 3.1 Create `reconcile-ingestion-filtering-runtime-contract` for the
  canonical CLI, overrides, dry-run, rerun, projection, feedback, and
  observability decisions.
- [x] 3.2 Create `closeout-db-source-overrides-evidence` for request-contract
  alignment, component/browser coverage, documentation, and migration proof.
- [x] 3.3 Create `operationalize-llm-evaluation-routing` for DB-effective
  config, classifier injection, paired datasets, calibration/enablement,
  failure/cost semantics, and truthful CLI/API/docs.
- [x] 3.4 Create `verify-production-paradedb-langfuse` for canonical image
  identity, Railway digest/extensions/search proof, Langfuse trace correlation,
  and rollback.
- [x] 3.5 Add minimal acceptance-aligned delta requirements to the six retained
...
- [x] 4.1 Manually merge only evidenced filtering, source-override,
...
- [x] 4.2 Archive `unify-mcp-ingest-envelope` as superseded with
  `--skip-specs`; do not repair or sync its obsolete synchronous shape.
- [x] 4.3 Manually merge the evidenced RI-01 CLI/test requirements and archive
  it with `--skip-specs`; manually create the reviewed RI-02 frontend release
  capability and archive RI-02 with `--skip-specs` to avoid a duplicate merge.
- [x] 4.4 Archive the analysis-only `feynman-inspired-features` entry with
  validation and spec syncing explicitly skipped, retaining `analysis.md`.
- [x] 4.5 Correct stale MCP inventory documentation that still recommends the
  superseded flat response or completed broad proposals.

## 5. Inventory proof and completion

- [x] 5.1 Create a final-state disposition snapshot and validator. Before
  ...
  archive sets.
- [x] 5.2 Validate every retained active change, all four successors, every
  touched main capability, the reconciliation change, and the package graph.
- [x] 5.3 Record review findings, validation evidence, exact test counts,
  environment-limited checks, and the final active inventory.
- [x] 5.4 Sync `openspec-inventory-governance`, archive this reconciliation
