...
1. DoclingParser processes PDF, DOCX, and image files
2. Tables extracted as structured data with >90% accuracy
3. OCR extracts text from scanned documents when enabled
4. File upload API accepts files up to configured size limit
5. Duplicate files detected and linked to canonical version
6. Router selects Docling for PDF/image when available
7. Fallback to MarkItDown works when Docling unavailable
8. All metadata fields populated when available in source
