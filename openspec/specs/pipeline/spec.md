...
The pipeline SHALL execute all ingestion sources (Gmail, RSS, YouTube, Podcast) concurrently rather than sequentially.

#### Scenario: All sources succeed
- **WHEN** the `aca pipeline daily` command is executed
- **THEN** all 4 sources are fetched concurrently via `asyncio.gather()`
