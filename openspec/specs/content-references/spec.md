...
#### Scenario: Every reference carries at least one identifier
```
GIVEN the content_references table exists
WHEN a row is inserted with both external_id and external_url NULL
THEN the chk_has_identifier CHECK constraint rejects the insert
AND a row supplying either identifier alone is accepted
```
...
The system SHALL provide a queue-based background job (`resolve_references`) that:
...
#### Scenario: Unresolved references are resolved in batches
```
GIVEN unresolved content_references rows exist
WHEN the resolve_references background job runs
THEN it matches targets via GIN-indexed metadata_json containment and source_url
AND sets target_content_id, resolution_status and resolved_at on each match
AND processes at most the configured batch size (default 100) per pass
```

### Requirement: Reverse Resolution on Ingestion

When new content is ingested, the system SHALL check for existing unresolved references that match:
...
Resolved references SHALL be projected to Neo4j as edges:
...
When `reference_auto_ingest_enabled` is set, the system SHALL auto-ingest content
for unresolved references carrying a structured identifier, and SHALL bound that
recursion by tracked depth.
...
The system SHALL link Scholar and arXiv records describing the same paper with a
`supplements` reference in each direction.
...
The API SHALL expose both directions of the citation graph for a content item.
...
The CLI SHALL expose reference backfill and manual resolution passes.
...
The migration SHALL create the `content_references` table and its indexes without
introducing PostgreSQL enum types.
...
#### Scenario: Migration avoids PostgreSQL enum types
```
GIVEN the content_references migration is applied
WHEN reference_type, external_id_type and resolution_status columns are created
THEN they are VARCHAR(20) validated by application-level StrEnum
AND no PostgreSQL enum type is created for them
AND uq_content_reference_url is a partial unique index with a WHERE clause
AND contents.metadata_json is jsonb with a GIN index
```

### Requirement: Settings

Reference behavior SHALL be controlled by the following settings.
...
#### Scenario: Reference behavior is settings-gated
```
GIVEN the reference settings are at their defaults
WHEN content is ingested
THEN extraction runs (reference_extraction_enabled = True)
AND Neo4j projection runs (reference_neo4j_sync_enabled = True)
AND auto-ingest does NOT run (reference_auto_ingest_enabled = False)
AND references below reference_min_confidence (0.5) are not stored
```

### Requirement: Ingestion Hook

All ingestion services SHALL call reference extraction after creating/updating a Content record:
...
All API endpoints and CLI commands SHALL also be registered as MCP tools in `src/mcp_server.py` via `@mcp.tool()`, following the existing delegation pattern:
...
#### Scenario: Every API and CLI capability is reachable over MCP
```
GIVEN the MCP server is running
WHEN an MCP client lists available tools
THEN get_content_references, extract_references, resolve_references and
     ingest_reference are registered via @mcp.tool()
AND ingest_reference operates independently of reference_auto_ingest_enabled
```
