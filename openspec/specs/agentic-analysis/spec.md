...
The conductor SHALL accept tasks from users and from the scheduler, plan them
against the active persona and prior memory, delegate sub-tasks to specialists,
and drive each task through its status lifecycle to completion or failure.
...
### Requirement: Specialist Agents

The system SHALL provide research, analysis, synthesis, and ingestion
specialists, each invoked by the conductor for the portion of a task matching
its capability.


#### Scenario: Research specialist performs deep investigation
...
#### Scenario: Analysis specialist detects trends
...
#### Scenario: Synthesis specialist generates insights
...
#### Scenario: Ingestion specialist acquires content
...
The system SHALL persist and recall agent memory through a configurable
provider, supporting a hybrid recall strategy across prior tasks and insights.
...
### Requirement: Proactive Scheduler

The system SHALL execute agent tasks on a cron schedule declared in
`settings/schedule.yaml`, and SHALL support managing and source-filtering those
schedules.
...
#### Scenario: Schedule-level source filtering
...
The system SHALL classify task risk and require human approval before executing
actions above the configured risk threshold, honoring per-persona overrides.
...
The system SHALL load persona configuration from `settings/personas/`, apply it
to conductor and specialist execution, and expose the available personas.


#### Scenario: Multi-persona loading and application
...
The API SHALL expose task submission and status tracking, and SHALL stream task
progress over SSE.


#### Scenario: Task submission and tracking
...
#### Scenario: SSE progress streaming
...
The CLI SHALL expose task submission, status, insight browsing, persona listing,
and schedule management under the `aca agent` group.


#### Scenario: Agent CLI interface
...
### Requirement: Measurement & Thresholds

The system SHALL score confidence, enforce iteration and cost budgets, rank
fused results by Reciprocal Rank Fusion, and apply a bounded retry policy.
...
#### Scenario: Iteration and cost budgets
...
#### Scenario: Reciprocal Rank Fusion (RRF) formula
...
Specialist failures, tool failures, memory-backend unavailability, approval
timeouts, and task timeouts SHALL each be handled explicitly and SHALL leave the
task in a terminal, inspectable state.


#### Scenario: Specialist execution failure
...
#### Scenario: Tool execution failure within specialist
...
#### Scenario: Memory backend unavailability
...
The agent surface SHALL preserve LLMRouter backward compatibility and SHALL
integrate with the pipeline runner and the durable job queue's `agent_task`
entrypoint.


#### Scenario: LLMRouter backward compatibility
...
#### Scenario: Pipeline Runner integration
...
#### Scenario: Job queue agent_task entrypoint
...
### Requirement: LLMRouter Extensions

The LLMRouter SHALL support enhanced tool calling and a planning method that
decomposes a goal into executable sub-tasks.


#### Scenario: Enhanced tool calling
