...
class TestSecurityHeaders:
    ...
    def test_server_header_not_overly_detailed(
        ...
        for detail in ("ubuntu", "debian", "centos", "alpine", "amazon"):
            assert detail not in server, f"Server header reveals OS detail: {server}"

    def test_no_powered_by_header(self, client: httpx.Client, health_endpoint: str) -> None:
    ...
    def test_content_type_options_nosniff(
        ...
        if xcto is None:
            pytest.skip("X-Content-Type-Options not set — consider adding 'nosniff' via middleware")
